...
* extrude_speed: 挤出的速度
* drip_extrude_distance: charge 时挤出机和 ViViD 同时一起进料的单次距离, 例如 `drip_extra_distance = 10`, `drip_extrude_distance = 1` 结果是单次进料 1mm, 总共进料最多 10 次(为了避免啃料, 如果 Outlet 传感器提前触发了则会提前中止)
* drip_extra_distance: charge 时挤出机和 ViViD 同时一起进料的最大距离(为了避免啃料, 如果 Outlet 传感器提前触发了则会提前中止)
